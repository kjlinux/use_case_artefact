...
from src.ingestion.minio_client import read_csv_from_minio
from src.ingestion.transformer import transform_and_split
from src.ingestion.postgres_loader import load_to_postgres
...
def main():
    ...
    parser.add_argument("date", type=parse_date, help="Date de vente a ingerer (YYYYMMDD)")
    args = parser.parse_args()

    target_date = args.date.date()
    logger.info(f"Ingestion demarree pour {target_date}")
    ...
    try:
        ...
        if tables is None:
            logger.warning(f"Aucune donnee pour {target_date}")
            sys.exit(0)

        logger.info(f"{len(tables['sale_items'])} articles a charger")
